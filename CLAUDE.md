...
- **API:** `saveEpaTargets(caseId, targets)` / `getEpaTargets(caseId)` / `clearEpaTargets(caseId)` / `getAllEpaTargets()` in `client/lib/assessmentStorage.ts` — encrypted with K_user like every other blob in that module; saving an empty list removes the key (stale targets clear on edit-save); the reader is v2-only (v1/plaintext records dropped + key removed; regenerate on next save); `@opus_epa_targets_index` mirrors the revealed index for batch loading
- **Derivation:** runs on case save via `deriveEpaAssessments()` (unit-based algorithm in `client/lib/epaDerivation.ts` — steps authoritative, procedure fallback with the owner's resolved role; see the Phase 3 bullet); errors are logged to Sentry + `__DEV__` console, never swallowed silently
