...
- **Per-Procedure Team + EPA Phase 1 COMPLETE (2026-07-27)** — Branch `feature/per-procedure-team-epa` (uncommitted at time of writing). First of 3 phases from `.claude/plans/i-just-logged-a-virtual-mccarthy.md` (per-step operative team attribution + EPA loop repair, triggered by a two-team free flap case where per-procedure team assignment was unreachable and EPAs never appeared). Bug-fix tier only — no data-model change:
  ...
  - ~~Phases 2–3 pending~~ Phase 2 shipped same day (next bullet); Phase 3 (EPA rewrite + full loop) still pending.
...
- **API:** `saveEpaTargets(caseId, targets)` / `getEpaTargets(caseId)` / `clearEpaTargets(caseId)` in `client/lib/assessmentStorage.ts` — encrypted with K_user like every other blob in that module; saving an empty list removes the key (stale targets clear on edit-save); legacy plaintext records fail decryption and are dropped (regenerate on next save)
- **Derivation:** runs on case save via `deriveEpaAssessments()` (seniority-chain algorithm in `client/lib/epaDerivation.ts`); errors are logged to Sentry + `__DEV__` console, never swallowed silently
- **Failure mode:** non-blocking — EPA derivation errors do not prevent case save
- **Consumption gap (Phase 3 pending):** nothing reads `getEpaTargets` yet — the owner-facing pending-assessments surface ships with the EPA loop rewrite (see `.claude/plans/i-just-logged-a-virtual-mccarthy.md`); the learning-curve dashboard reads revealed assessment pairs, not EPA targets
