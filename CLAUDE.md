...
- **Team Sharing Linking Overhaul COMPLETE (2026-07-23)** — Bumps app to 2.13.0. Implements the FULL 9-item plan from `.claude/plans/team-sharing-linking-improvement.md` (Phases 1–3, user-approved) on `feature/team-sharing-linking`, one gated commit per step — PLUS one bug discovered during implementation recon and fixed inside the extraction: **edit-saving an already-shared case silently 500'd against `shared_cases`' UNIQUE(caseId, recipientUserId)** (plain insert in `storage.createSharedCase`, error swallowed client-side), so edited case content NEVER reached existing recipients. `shareCaseWithTeam` now revokes still-present recipients BEFORE re-POSTing (envelopes cascade-delete with the row; fresh row wraps the fresh per-save case key; `verificationStatus` deliberately resets to pending — content changed). Push re-fires "Case Shared" on edit-save (accepted v1 cost; future: server upsert + "case updated" push type).
  ...
  - **Known gap flagged (spawn task)**: `profiles.discoverable` gates search/discovery/link server-side but NO endpoint or UI can change it (`profileUpdateSchema` omits it) — every account is permanently discoverable-by-default; the invited-link 404 copy deliberately avoids referencing a Settings toggle.
