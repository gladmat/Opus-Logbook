...
- **Version:** 2.7.0, buildNumber 11 (app.json) — but `eas.json` has `appVersionSource: "remote"` so EAS's stored appVersion is the authoritative source for the TestFlight metadata. As of the 2.7.0 commit `2d9012a`, the EAS remote appVersion is still **2.5.0** (last set during Phase 5 — bumping app.json doesn't propagate). Run `eas build:version:set --platform ios` (interactive) or update via the EAS dashboard to align the next build's TestFlight metadata. The 2.7.0 build itself ships as `2.5.0 build 1.2.60` on TestFlight; the code is fully 2.7.0. 2.7.0 is the Case Form UX Overhaul release (Phase 7); 2.6.0 was the security remediation (Phase 6, Sessions 1–7).
...
- **Framework:** Vitest 4.0.18, **1529 tests** across 86 files (+15 new MFC tests in Phase 7 / Case Form UX Overhaul)
