...
- **Follow-up Interval Auto-Derivation + Gallery Cleanup Everywhere COMPLETE (2026-07-30)** — Bumps app to 2.16.0. Two on-device friction reports fixed plus one silent data-loss bug found en route; tests 2255 → 2293 across 142 files; tsc/lint/prettier clean; sim-verified E2E (interval derive/re-derive/override-sticky/edit-guard, gallery prompt → iOS system dialog → originals gone, encrypted copies intact).
  ...
  - Known pre-existing quirk flagged (NOT fixed): backdated timeline events display one day late in UTC+12/+13 (`toUtcNoonIsoTimestamp` = next-day local midnight in NZ; display sites format `createdAt` with local TZ).
