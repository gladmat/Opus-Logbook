...
- **Phase 7 / Case Form UX Overhaul COMPLETE** — Bumps app to 2.7.0 (commit `2d9012a`). Pre-TestFlight audit-driven overhaul covering data-integrity ship blockers, accessibility, design-system cleanup, and IA fixes. Audit findings in `.claude/plans/case-form-ux-audit.md`, locked-in plan in `.claude/plans/case-form-ux-implementation.md`, post-TestFlight follow-up queue in `.claude/plans/case-form-followup-notes.md`. Tests 1514 → 1529 (+15 MFC); `tsc --noEmit` clean. Highlights:
  ...
  - **Scroll-jump preserve hook COMPLETE** — new `client/hooks/useScrollPreserve.ts` (`snapshot()` + `restore()` pair, no-op outside `FormScrollProvider`) wraps `CollapsibleFormSection.toggle` so collapsing a section above the viewport no longer drifts the visible content. `CaseFormScreen` gained `preReviewScrollRef` so Review → Back-to-Edit restores scroll position instead of re-mounting at 0. `handleScroll` switched to functional `setActiveSection` for `Object.is` bailout. Dev-only `__DEV__` jump detector (`>250px in <200ms`) logs `[opus:scroll-jump]` with `lastTouchedTestID` (fiber-walk on the root scroll view's `onTouchStart`) — left in for TestFlight field reports; strip on next session if not surfacing anything.
