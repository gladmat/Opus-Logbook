...
- **EPA Role-Gated Trigger + Bottom-Up Instrument v2 + Phase C COMPLETE (2026-08-23)** — Bumps app to 2.23.0. Implements the research findings (O-SCORE / Zwisch / ten Cate ES / SIMPL / BID / Essandoh 2025) on when the entrustment assessment should fire and what the trainee rates. Branch `feature/epa-role-gate-instrument-v2`; tests 2477 → 2541 across 157 files; tsc/lint/prettier clean. Full rationale + live claims in `docs/EPA-ARCHITECTURE.md` (§§1, 3.3, 3.6, 5, 6, 7, 9 updated).
  ...
  - **Not sim-verified this session** (unit-tested + tsc/lint clean): two-account E2E of the FA → exposure vs PS → assess flows, the v2 form, reveal cards, and Training tab should be spot-checked per the verification recipe in the plan file before the TestFlight cut. Residuals: version skew (old counterpart app may still commit FA pairs — gate rescue + PS-only curves mitigate), FA "what did you do" chip deferred, EPA-ARCH Phases A/D still deferred.
