...
export const TBSARegionalBreakdown = React.memo(function TBSARegionalBreakdown({
  data,
  onChange,
}: TBSARegionalBreakdownProps) {
      ...
      {validation && !validation.valid ? (
        ...
        >
          ...
          <ThemedText style={[styles.validationText, { color: theme.warning }]}>
            Regional total differs from quick entry by{" "}
            {validation.diff.toFixed(1)}%
          </ThemedText>
        </View>
      ) : null}
        ...
        return (
          ...
          >
            <View style={styles.regionHeader}>
              ...
              <ThemedText
                style={[styles.regionPercentage, { color: theme.link }]}
              >
                {entry?.percentage ?? 0}%
              </ThemedText>
            </View>
            ...
            <View style={styles.stepperRow}>
              ...
              {DEPTH_OPTIONS.map((depth) => {
                const isSelected =
                  (entry?.depth ?? "superficial_partial") === depth;
                return (
                  <TouchableOpacity
                    key={depth}
                    onPress={() => {
