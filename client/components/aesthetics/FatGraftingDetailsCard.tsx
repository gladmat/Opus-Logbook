...
const HARVEST_TECHNIQUES: {
  value: AestheticFatGraftingDetails["harvestTechnique"];
  label: string;
}[] = [
...
const PROCESSING_METHODS: {
  value: AestheticFatGraftingDetails["processingMethod"];
  label: string;
}[] = [
...
const FAT_TYPES: {
  value: AestheticFatGraftingDetails["fatType"];
  label: string;
}[] = [
...
const INJECTION_TECHNIQUES: {
  value: NonNullable<AestheticFatGraftingDetails["injectionTechnique"]>;
  label: string;
}[] = [
...
export const FatGraftingDetailsCard = React.memo(
  function FatGraftingDetailsCard({
    details,
    onChange,
    ...
    const update = useCallback(
      (partial: Partial<AestheticFatGraftingDetails>) =>
        onChange({ ...d, ...partial }),
      [d, onChange],
    );
    ...
    const handleAddRecipientSite = useCallback(
      (siteName: string) => {
        Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Light);
        const exists = d.recipientSites.some((s) => s.site === siteName);
        if (exists) {
          update({
            recipientSites: d.recipientSites.filter((s) => s.site !== siteName),
          });
        } else {
          update({
            recipientSites: [
              ...d.recipientSites,
              { site: siteName, volumeInjectedMl: 0 },
            ],
          });
        }
      },
    ...
    const handleSiteVolumeChange = useCallback(
      (siteName: string, vol: number) => {
        update({
    ...
    const selectedSiteNames = useMemo(
      () => new Set(d.recipientSites.map((s) => s.site)),
      [d.recipientSites],
    );
    ...
    return (
      <View style={styles.container}>
            ...
            {HARVEST_TECHNIQUES.map((t) => {
              ...
              return (
                <Pressable
                  ...
                  onPress={() => {
                    Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Light);
                    update({ harvestTechnique: t.value });
                  }}
                  style={chipStyle(sel)}
                >
        ...
        <View style={styles.switchRow}>
          ...
          <Switch
            value={d.tumescentUsed}
            onValueChange={(v) => update({ tumescentUsed: v })}
            trackColor={{ true: theme.link }}
          />
        </View>
        <NumRow
          label="Total aspirate (mL)"
          value={d.totalAspirateMl}
          onChange={(v) => update({ totalAspirateMl: v })}
          theme={theme}
        />
            ...
            {PROCESSING_METHODS.map((m) => {
              ...
              return (
                <Pressable
                  ...
                  onPress={() => {
                    Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Light);
                    update({ processingMethod: m.value });
                  }}
                  style={chipStyle(sel)}
                >
                  <ThemedText style={chipTextStyle(sel)}>{m.label}</ThemedText>
                </Pressable>
              );
            })}
          </View>
        ...
        <NumRow
          label="Processed volume (mL)"
          value={d.processedVolumeMl || undefined}
          onChange={(v) => update({ processedVolumeMl: v ?? 0 })}
          theme={theme}
        />
        <View style={styles.section}>
          <ThemedText style={[styles.label, { color: theme.textSecondary }]}>
            ...
            {FAT_TYPES.map((f) => {
              ...
              return (
                <Pressable
                  ...
                  onPress={() => {
                    Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Light);
                    update({ fatType: f.value });
                  }}
                  style={chipStyle(sel)}
                >
                  <ThemedText style={chipTextStyle(sel)}>{f.label}</ThemedText>
                </Pressable>
              );
            })}
          </View>
          ...
          <View style={styles.chipRow}>
            {RECIPIENT_SITE_OPTIONS.map((s) => {
              ...
              return (
                <Pressable
                  key={s}
                  onPress={() => handleAddRecipientSite(s)}
                  style={chipStyle(sel)}
                >
                  <ThemedText style={chipTextStyle(sel)}>{s}</ThemedText>
                </Pressable>
            ...
            >
              ...
              <TextInput
                ...
                onChangeText={(t) => {
                  ...
                  handleSiteVolumeChange(rs.site, isNaN(n) ? 0 : n);
                }}
                placeholder="mL"
            ...
            {INJECTION_CANNULAS.map((g) => {
              ...
              return (
                <Pressable
                  ...
                  onPress={() => {
                    Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Light);
                    update({ injectionCannulaGauge: sel ? undefined : g });
                  }}
                  style={chipStyle(sel)}
                >
            ...
            {INJECTION_TECHNIQUES.map((t) => {
              ...
              return (
                <Pressable
                  ...
                  onPress={() => {
                    Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Light);
                    update({ injectionTechnique: sel ? undefined : t.value });
                  }}
                  style={chipStyle(sel)}
                >
                  <ThemedText style={chipTextStyle(sel)}>{t.label}</ThemedText>
