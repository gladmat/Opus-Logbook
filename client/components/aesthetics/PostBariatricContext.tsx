...
const TIME_SINCE_OPTIONS: {
  value: TimeSinceBariatricSurgery;
  label: string;
}[] = [
...
export const PostBariatricContext = React.memo(function PostBariatricContext({
  ...
  const weightLoss = useMemo(() => {
    if (
      assessment.preBariatricWeightKg != null &&
      assessment.currentWeightKg != null
    ) {
  ...
  const bmi = useMemo(() => {
    if (
      ...
      return (
        Math.round((assessment.currentWeightKg / (heightM * heightM)) * 10) / 10
      );
    }
    return undefined;
  ...
  const twlPercent = useMemo(() => {
    if (
      weightLoss != null &&
      assessment.preBariatricWeightKg != null &&
      assessment.preBariatricWeightKg > 0
    ) {
  ...
  const pittsburghTotal = useMemo(() => {
    ...
    const scores = Object.values(assessment.pittsburghRatingScale.scores) as (
      | PittsburghScore
      | undefined
    )[];
  ...
  const handlePittsburghScore = useCallback(
    (region: PittsburghRegion, score: PittsburghScore) => {
      ...
      const total = (
        Object.values(scores) as (PittsburghScore | undefined)[]
  ...
  return (
    <View style={[styles.container, { borderColor: theme.info + "40" }]}>
      ...
      <View style={styles.row}>
        <NumField
          label="Pre-bariatric (kg)"
          value={assessment.preBariatricWeightKg}
          onChange={(v) => update({ preBariatricWeightKg: v })}
          theme={theme}
        />
        <NumField
          label="Current (kg)"
            ...
            return (
              <Pressable
                ...
                onPress={() => {
                  ...
                  update({ bariatricProcedureType: sel ? undefined : p.value });
                }}
                style={chipStyle(sel)}
              >
                <ThemedText style={chipTextStyle(sel)}>{p.label}</ThemedText>
              </Pressable>
            );
          })}
        </View>
            ...
            return (
              <Pressable
                ...
                onPress={() => {
                  Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Light);
                  update({
                    timeSinceBariatricSurgery: sel ? undefined : t.value,
                  });
                }}
                style={chipStyle(sel)}
              >
      ...
      <View style={styles.switchRow}>
        ...
        <Switch
          value={assessment.weightStable ?? false}
          onValueChange={(v) => update({ weightStable: v })}
      ...
      >
        <View style={styles.pittsburghHeaderLeft}>
          <ThemedText style={[styles.pittsburghTitle, { color: theme.text }]}>
          ...
          {pittsburghTotal != null && (
            ...
            >
              <ThemedText
                style={[styles.scoreBadgeText, { color: theme.link }]}
              >
                {pittsburghTotal}/30
              </ThemedText>
            </View>
        ...
        <Feather
          name={pittsburghExpanded ? "chevron-up" : "chevron-down"}
          size={18}
      ...
      {pittsburghExpanded && (
        <View style={styles.pittsburghGrid}>
          {PITTSBURGH_REGIONS.map(({ value: region, label }) => {
            const currentScore =
              assessment.pittsburghRatingScale?.scores?.[region];
            return (
              <View key={region} style={styles.pittsburghRow}>
                <ThemedText
                  style={[styles.pittsburghRegionLabel, { color: theme.text }]}
                  numberOfLines={1}
                >
                  {label}
                </ThemedText>
...
function NumField({
  ...
  return (
    <View style={styles.numField}>
      <ThemedText
        style={[styles.numFieldLabel, { color: theme.textSecondary }]}
      >
        {label}
      </ThemedText>
      <TextInput
        style={[
          styles.numFieldInput,
          {
